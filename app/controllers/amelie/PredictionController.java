...
public class PredictionController extends Controller {
    ...
    private int computeCorrectMatch(ArrayNode decisionsToPredict, int run, Set<String> allRecommendedExpertsInTestingSet) {
        ...
        for(int n=0; n <decisionsToPredict.size(); n++) {
            ...
            int max = dtp_pa.size();
            if(run != 0) {
                max = dtp_pa.size() > (run * 1) ? (run * 1) : dtp_pa.size();
            ...
            pcvja.forEach(pcvjo -> {
                ...
                for (int k = 0; k < conceptSize; k++) {
                    if (cv.get(k).asInt(0) > 0 && pcvList.get(k).asInt(0) > 0) {
                        ...
                        newpcvList.add(cv.get(k).asInt(0) * pcvList.get(k).asInt(0));
                        isApplicable = true;
                    } else {
                        newpcvList.add(0);
                    }
        ...
        testingData.forEach(issue -> {
            ...
            concepts.forEach(concept -> {
                ...
                if(conceptList.contains(c)) {
                    int value = StaticFunctions.getConceptValue(c, issue.get(StaticFunctions.SUMMARY).asText("") + " " + issue.get(StaticFunctions.DESCRIPTION).asText(""));
                    int i = conceptList.indexOf(c);
                    conceptVector.remove(i);
                    conceptVector.insert(i, value);
                }
            });
